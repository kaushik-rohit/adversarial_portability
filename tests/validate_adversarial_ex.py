...
gpu_ids = [1, 4, 5, 6]
n_GPU = len(gpu_ids)
pbar = tqdm(total= len(model_names) * int(10000 / BATCH_SIZE))

columns = ['target', 'incorrectly classified samples']
...
def classification_on_target(rank, target, attack, alpha):
    ...
    for data in data_loader:
        ...
        print((true_label == preds).long())
        n = preds.size(dim=0) - torch.sum((preds == true_label).long()).item()
...
def caculate_adversary_with_fixed_alpha():
    for attack in attacks:
        for alpha in alphas:
            ...
            res_df.to_csv('verify_{}_alpha={}.csv'.format(attack, alpha), index=False)
